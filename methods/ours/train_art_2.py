...
# import wandb
writer = SummaryWriter(log_dir='erfnet_0.5_art_finetune')
...
parser.add_argument('--finetune_layers', choices=['all', 'fc', 'classifier'], default='all',
...
# Paths.
parser.add_argument('--dataset', type=str, default='art', help='Name of dataset')
parser.add_argument('--dataset_mask', type=str, default='salicon', help='Name of dataset')
...
parser.add_argument('--prune_perc_name', type=float, default=0.5, help='% of neurons to prune per layer')
...
class Manager(object):
    ...
    def __init__(self, args, model, model_old, previous_masks, masks_arch_current, dataset2idx, dataset2biases):
        ...
        if args.mode != 'check':
            ...
            self.train_loader, self.test_loader = create_art_data_loaders(args,
        ...
        for name, m in self.model.named_parameters():
            if 'decoder' in name:
                ...
                if 'bn' in name or 'parallel_conv' in name:
                    if '.{}.weight'.format(self.args.current_task) in name or '.{}.bias'.format(
                            self.args.current_task) in name:
                        continue
                    else:
                        m.requires_grad = False
        min_loss = min_loss
    ...
    def prune(self):
        ...
        if self.args.post_prune_epochs:
            ...
            optimizer = Adam(self.model.parameters(), lr=self.args.lr, betas=(0.9, 0.999), eps=1e-08,
...
def main():
    ...
    if args.mode == 'prune':
        dict_load = ckpt['model_state_dict']
        model.load_state_dict(dict_load)
        print('loaded model from checkpoint provided.')
    ...
    if args.mode == 'finetune':
        ...
        if args.finetune_layers == 'all':
            ...
            # print(params_to_optimize.requires_grad)
        ...
        # Perform finetuning.
        manager.train(args.finetune_epochs, optimizer, save=True,
                      savename=args.save_prefix + args.arch + '_' + args.dataset + '_finetune_' + str(
                          args.prune_perc_per_layer))
